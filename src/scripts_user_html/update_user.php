...
<html>
<body>
<h1>Formulario para la modificación de usuarios</h1>
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ((trim($email) != '') && !validar_email($email)) {
        $errores[] = 'El campo email tiene un formato no válido.';
    }

    if ((trim($enabled) != '') && !validar_entero($enabled)) {
        $errores[] = 'El campo enabled debe ser un número.';
    }

    if ((trim($enabled) != '') && !validar_limite_entero($enabled)) {
    ...
    if (empty($errores)) {
        ...
        /** Se comprueba que exista el usuario que se intenta modificar */
        /** @var User $user */
        $userId = (int)trim($id);
        $user = $entityManager
            ...
            ->findOneBy(['id' => $userId]);
        if (empty($user)) {
            echo '<h3 style="color: #f45642;">' . 'Usuario con ID #' . $userId . 'no encontrado.' . '</h3>' . PHP_EOL;
        } else {
            ...
            try {
                ...
                $entityManager->flush();
                echo '<h3 style="color: #5042f4">' . 'Modificado User with ID #' . $user->getId() . '</h3>' . PHP_EOL;
            } catch (Exception $exception) {
